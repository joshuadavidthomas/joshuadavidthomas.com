...
class YamdlConfig(AppConfig):
    ...
    def ready(self):
        ...
        if not self.loaded:
            ...
            if not self.connection.is_in_memory_db():  # type: ignore[attr-defined]
                raise ImproperlyConfigured(
                    "The Yamdl database must be shared in-memory SQLite"
                )
            ...
            self.loader_class = import_string(
                getattr(settings, "YAMDL_LOADER", "core.yamdl.loader.ModelLoader")
            )
