...
(function (global, factory) {
  ...
  const nativeAPI = (() => {
    ...
    for (const methodList of methodMap) {
      const exitFullscreenMethod =
        methodList === null || methodList === void 0 ? void 0 : methodList[1];
  ...
  const Plugin = () => {
    ...
    const siblings = (elem) => {
      ...
      let sibling =
        elem.parentNode.firstElementChild || elem.parentNode.firstChild;

      while (sibling) {
        if (sibling !== elem && sibling.nodeType === Node.ELEMENT_NODE)
    ...
    const fullScreenEffects = function (deck, options) {
      ...
      if (options.compatibility) {
        console.log(
          "FsFx runs in compatibility mode. Please go to https://github.com/martinomagnifico/reveal.js-fsfx to check the recent changes. Compatibility mode can be turned off if you are sure that fullscreen buttons are displayed correctly.",
        );
      }

      let viewport =
        deck.getRevealElement().tagName == "BODY"
          ? document
      ...
      if (options.auto) {
        if (options.auto.generate) {
          ...
          slideSiblings.forEach((slideSibling) => {
            if (
              slideSibling.classList.contains(options.baseclass) ||
              slideSibling.querySelector(`.${options.baseclass}`)
            ) {
        ...
        if (options.auto.position) {
          ...
          if (deck.getConfig().embedded == true) {
            ...
            let unid = "";
            unid = deck.getRevealElement().id
              ? deck.getRevealElement().id
              : generateID();
            thisid = "#" + unid + ".reveal";
          ...
          positions.top = options.auto.position.top
            ? options.auto.position.top
            : "auto";
          positions.right = options.auto.position.right
            ? options.auto.position.right
            : "auto";
          positions.bottom = options.auto.position.bottom
            ? options.auto.position.bottom
            : "auto";
          positions.left = options.auto.position.left
            ? options.auto.position.left
            : "auto";
        ...
        fsButton.onclick = function () {
          if (fsButton.dataset.fsGonext && fsButton.dataset.fsGonext > 0) {
            if (screenfull$1.isFullscreen || !sfEnabled()) {
              ...
              screenfull$1.request(viewport).then(
                setTimeout(function () {
                  deck.next();
      ...
      const toggleCheck = function (toggleThese) {
        ...
        if (sfEnabled() == true) {
          document.addEventListener(
            screenfull$1.raw.fullscreenchange,
            fullscreenchange,
          );
        }
      };
